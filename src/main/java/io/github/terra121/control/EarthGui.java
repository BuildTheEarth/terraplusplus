...
public class EarthGui extends GuiScreen implements DynamicOptions.Handler {
    ...
    public EarthGui(GuiCreateWorld guiCreateWorld, Minecraft mc) {
        ...
        settingElems = new DynamicOptions.Element[]{
                cycleButton(6969, "projection", projs, e -> {
                    projectMap(true);
                    return I18n.format("terra121.gui.projection") + ": " + I18n.format("terra121.projection." + e);
                }),
                cycleButton(6968, "orentation", GeographicProjection.Orientation.values(), e -> {
                    projectMap(true);
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                if (bounds[0] <= X && X <= bounds[2] && bounds[1] <= Y && Y <= bounds[3]) {
                    ...
                    if (biomemap != null) {
                        img.setRGB(x, y, biomemap.getColor(proj)); //biome map
                    } else { //image map
