...
public final class TransformedGridPointArray2D extends AbstractPointArray2D {
    ...
    TransformedGridPointArray2D(@NonNull PointArray parent, @NonNull CoordinateReferenceSystem crs, @NonNull CoordinateOperation operation) {
        ...
        checkArg(((LenientComparable) operation.getSourceCRS()).equals(parent.crs(), ComparisonMode.IGNORE_METADATA), "parent crs=%s, operation source crs=%s", parent.crs(), operation.getSourceCRS());
        checkArg(((LenientComparable) operation.getTargetCRS()).equals(crs, ComparisonMode.IGNORE_METADATA), "this crs=%s, operation target crs=%s", crs, operation.getTargetCRS());
    ...
    public double[] estimatedPointDensity() {
        ...
        return new double[]{
                parentDensity[0] * selfEnvelope.getSpan(0) / parentEnvelope.getSpan(0),
                parentDensity[1] * selfEnvelope.getSpan(1) / parentEnvelope.getSpan(1),
        };
    }
}
